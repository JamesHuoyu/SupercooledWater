...
class PropensityFieldClusters:
    ...
    # ------------------------------------------------------------------
    # Step 1: Gaussian density field
    # ------------------------------------------------------------------

    def _step1_build_density_field(self):
        """
        ρ_P(r) = Σ_i w_i · K_L(|r − r_i^0|)

        Here K_L(r) is an isotropic exponential kernel,

            K_L(r) = exp(-r / L),

        implemented as a periodic convolution on a 3D grid.
        """

        # Active-molecule weights
        if self.propensity_threshold is not None:
            p_thr = self.propensity_threshold
        else:
        ...
        # Deposit weights onto grid using nearest-grid-point assignment
        rho0 = np.zeros((self._nx, self._ny, self._nz), dtype=float)

        ix = np.clip((self.positions[:, 0] / self._hx).astype(int),
                    0, self._nx - 1)
        iy = np.clip((self.positions[:, 1] / self._hy).astype(int),
                    0, self._ny - 1)
        iz = np.clip((self.positions[:, 2] / self._hz).astype(int),
                    0, self._nz - 1)

        np.add.at(rho0, (ix, iy, iz), weights)

        # Exponential-kernel convolution
        # L should be in Å if positions/grid_spacing are in Å.
        ...
        rho = irfftn(
            rfftn(rho0) * rfftn(kernel),
            s=rho0.shape
        ).real

        self.results.rho_field = rho
        self.results.grid_shape = rho.shape
        self.results.grid_spacing_actual = np.array(
    ...
    def _step2_compute_threshold(self):
        """
        Iterative background convergence (document §A.B):
          ρ_0 ← ρ_max
          repeat:
              ρ_0 ← mean(ρ[ρ ≤ ρ_0])
          until |Δρ_0| < tol
        ...
        if self.rho_threshold_mode == "iterative":
            ...
            for _ in range(500):
                ...
                if abs(rho_new - rho_0) / rho_new < tol:
                    rho_0 = rho_new
                    break
                rho_0 = rho_new
            rho_th = rho_0
    ...
    def _step3_label_clusters(self):
        """Label connected regions where ρ_P ≥ ρ_th."""
        rho     = self.results.rho_field
        rho_th  = self.results.rho_threshold
        mask    = rho >= rho_th

        # Full 3D connectivity (26-connected)
        ...
        self.results.cluster_labels = cleaned      # 3D array, 0 = background
    ...
    def _step5_characterise_clusters(self):
        """
        For each connected cluster region, find which OW atoms fall
        inside it and compute structural statistics from ζ_cg.
        """
        labels  = self.results.cluster_labels
        n_cl    = self.results.n_clusters
        h       = self.results.grid_spacing_actual   # [hx, hy, hz]

        # Pre-compute grid indices for all OW atoms
        ix = np.clip((self.positions[:, 0] / h[0]).astype(int), 0, self._nx - 1)
        iy = np.clip((self.positions[:, 1] / h[1]).astype(int), 0, self._ny - 1)
        iz = np.clip((self.positions[:, 2] / h[2]).astype(int), 0, self._nz - 1)
        atom_cluster_id = labels[ix, iy, iz]  # cluster id per atom (0 = BG)

        stats_list: List[ClusterStats] = []
        for cid in range(1, n_cl + 1):
            # Voxel volume
            vox_count = int((labels == cid).sum())
            # Atoms assigned to this cluster
            member_mask = atom_cluster_id == cid
            ...
            valid_z = zeta_m[~np.isnan(zeta_m)]
            ...
            stats = ClusterStats(
                ...
                mean_propensity  = float(np.mean(prop_m)),
                centroid         = centroid,
                rg               = rg,
        ...
        for rank, s in enumerate(stats_list):
            s.cluster_id = rank + 1   # re-index after sort

        self.results.cluster_stats      = stats_list
        self.results.atom_cluster_id    = atom_cluster_id  # per-atom cluster id
