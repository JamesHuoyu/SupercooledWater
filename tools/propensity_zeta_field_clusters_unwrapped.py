...
class PropensityFieldClusters:
    ...
    def __init__(
        ...
        # "unwrapped" is recommended for material/stress-bearing clusters:
        # positions are used as supplied and opposite box faces are not connected.
        # "wrapped" reproduces the old torus/PBC behaviour.
        coordinate_mode:      str   = "unwrapped",  # "unwrapped" or "wrapped"
        ...
        if self.coordinate_mode not in {"unwrapped", "wrapped"}:
            raise ValueError("coordinate_mode must be 'unwrapped' or 'wrapped'.")
        if self.coordinate_mode == "unwrapped" and self.periodic_labels:
            warnings.warn(
                "periodic_labels=True is inconsistent with coordinate_mode='unwrapped'; "
                "disabling periodic label merging."
            )
            self.periodic_labels = False

        self._setup_grid_geometry(grid_origin=grid_origin)

        # Results namespace (plain object)
        class _R: pass
        self.results = _R()

    def _setup_grid_geometry(self, grid_origin: Optional[np.ndarray] = None):
        """Set grid origin, extents, and spacing.

        In coordinate_mode='wrapped', the grid is the original simulation box
        [0, Lx) x [0, Ly) x [0, Lz). In coordinate_mode='unwrapped', the grid
        is a nonperiodic Cartesian box that spans the supplied unwrapped
        positions. This prevents particles crossing opposite box faces from
        being artificially merged into the same material cluster.
        """
        if self.coordinate_mode == "wrapped":
            self.grid_origin = np.zeros(3, dtype=float) if grid_origin is None else np.asarray(grid_origin, dtype=float)[:3]
            self.grid_lengths = self.box.copy()
        else:
            finite_pos = self.positions[np.all(np.isfinite(self.positions), axis=1)]
            if finite_pos.size == 0:
                raise ValueError("positions contains no finite 3D coordinates.")

            if grid_origin is None:
                lo = finite_pos.min(axis=0) - self.grid_padding
            else:
                lo = np.asarray(grid_origin, dtype=float)[:3]

            hi = finite_pos.max(axis=0) + self.grid_padding

            # Snap to grid-spacing multiples for stable plotting/edges.
            lo = np.floor(lo / self.grid_spacing) * self.grid_spacing
            hi = np.ceil(hi / self.grid_spacing) * self.grid_spacing
            lengths = np.maximum(hi - lo, self.grid_spacing)

            self.grid_origin = lo.astype(float)
            self.grid_lengths = lengths.astype(float)

        self.grid_upper = self.grid_origin + self.grid_lengths

        # Grid dimensions
        self._nx, self._ny, self._nz = [
            max(1, int(np.ceil(self.grid_lengths[d] / self.grid_spacing)))
            for d in range(3)
        ]

        # Actual voxel sizes (may differ slightly from grid_spacing)
        self._hx = self.grid_lengths[0] / self._nx
        self._hy = self.grid_lengths[1] / self._ny
        self._hz = self.grid_lengths[2] / self._nz
    ...
    def _grid_indices(self, positions: Optional[np.ndarray] = None):
        ...
        For coordinate_mode='unwrapped', positions are not folded back into the
        simulation box. For coordinate_mode='wrapped', the old PBC behaviour is
        retained by applying modulo box lengths before indexing.
        """
        ...
        if self.coordinate_mode == "wrapped":
            rel = np.column_stack((
                np.mod(pos[:, 0], self.box[0]),
                np.mod(pos[:, 1], self.box[1]),
                np.mod(pos[:, 2], self.box[2]),
            ))
        else:
            rel = pos - self.grid_origin[None, :]

        ix = np.clip((rel[:, 0] / self._hx).astype(int), 0, self._nx - 1)
        iy = np.clip((rel[:, 1] / self._hy).astype(int), 0, self._ny - 1)
        iz = np.clip((rel[:, 2] / self._hz).astype(int), 0, self._nz - 1)
        return ix, iy, iz
    ...
    def _deposit_cic(self, weights: np.ndarray) -> np.ndarray:
        ...
        In wrapped mode, interpolation wraps across opposite box faces.
        In unwrapped mode, interpolation is strictly nonperiodic; mass that
        would be assigned outside the grid is simply ignored. This avoids
        artificial cluster connections across the original periodic boundary.
        """
        field = np.zeros((self._nx, self._ny, self._nz), dtype=float)
        valid = np.isfinite(weights) & (weights != 0.0)
        if not np.any(valid):
            ...
            for dy, wy in ((0, 1.0 - fy), (1, fy)):
                ...
                for dz, wz in ((0, 1.0 - fz), (1, fz)):
                    ...
                    if self.coordinate_mode == "wrapped":
                        np.add.at(
                            field,
                            (ii % self._nx, jj % self._ny, kk % self._nz),
                            ww,
                        )
                    else:
                        inside = (
                        ...
                        if np.any(inside):
                            np.add.at(field, (ii[inside], jj[inside], kk[inside]), ww[inside])
        return field
    ...
    def _build_propensity_weights(self) -> Tuple[np.ndarray, float]:
        ...
        if self.propensity_threshold is not None:
            ...
            p_thr = np.nanpercentile(self.propensity, 85)
        s = 0.2 * (np.nanpercentile(self.propensity, 95) - np.nanpercentile(self.propensity, 50))
        weights = 1.0 / (1.0 + np.exp(-(self.propensity - p_thr) / s))
        # weights = np.maximum(self.propensity - p_thr, 0.0).astype(float)
        # weights[~np.isfinite(weights)] = 0.0
        # weights = np.log(self.propensity)

        return weights, p_thr

    def _build_propensity_density_field(self):
        """Build the propensity-density field.

        Wrapped mode keeps the original periodic FFT convolution. Unwrapped
        mode uses nonperiodic Gaussian smoothing with zero-density boundaries,
        so dynamic clusters are not connected through opposite box faces.
        """
        ...
        if self.coordinate_mode == "wrapped":
            ...
            self.results.smoothing_boundary = "periodic"
        else:
            sigma_grid = (
                max(self.exp_kernel_length / self._hx, 0.0),
                max(self.exp_kernel_length / self._hy, 0.0),
                max(self.exp_kernel_length / self._hz, 0.0),
            )
            rho = gaussian_filter(rho0, sigma=sigma_grid, mode="constant", cval=0.0)
            self.results.smoothing_boundary = "nonperiodic_constant"
    ...
    def _step4_refine_boundaries(self):
        ...
        for cid in range(1, n_cl + 1):
            ...
            lo_ang = self.grid_origin + lo_c * np.array([self._hx, self._hy, self._hz])
            hi_ang = self.grid_origin + hi_c * np.array([self._hx, self._hy, self._hz])
    ...
    def plot_slice(
        ...
        default_centre = self.grid_origin[axis] + 0.5 * self.grid_lengths[axis]
        centre = default_centre if slice_centre is None else slice_centre

        # Slab mask. Only wrapped mode applies PBC correction; unwrapped mode
        # uses the supplied coordinates directly.
    ...
    def plot_density_slice(
        ...
        with periodic wrapping only when coordinate_mode='wrapped'. In
        coordinate_mode='unwrapped', the supplied coordinates and grid are used
        directly. By default, ``slice_centre`` is the grid centre and
        ``slice_width=10.0`` Å, i.e. the displayed slab is centre ± 5 Å.
        ...
        default_centre = self.grid_origin[axis] + 0.5 * self.grid_lengths[axis]
        centre = default_centre if slice_centre is None else float(slice_centre)

        # Grid-cell centres along the slab-normal axis.
        n_axis = rho.shape[axis]
        centres_axis = self.grid_origin[axis] + (np.arange(n_axis) + 0.5) * h[axis]

        # Distance from the requested slice centre. Only wrapped mode uses a
        # minimum-image distance; unwrapped mode keeps the real coordinate.
        ...
        x_edges = self.grid_origin[ax_x] + np.arange(rho_2d.shape[0] + 1) * h[ax_x]
        y_edges = self.grid_origin[ax_y] + np.arange(rho_2d.shape[1] + 1) * h[ax_y]
